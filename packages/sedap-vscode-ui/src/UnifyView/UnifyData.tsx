...
import { UnifyMap, UnifySeg, UnifyStep } from "@sedap/types";
...
const UnifyData = ({ selectStep }: Props) => {
  const [{ path, unifications }] = useStore((store) => [
    store.unifyState,
  ...
  const unifyNames: [string, React.JSX.Element][] = [
    [
      "",
      <span>
        {title} ({subtitle})
  ...
  for (let i = path.length - 1; i > 0; i--) {
    ...
    if (
      ...
      unifyNames.push([assertion, <Code>{assertion}</Code>]);
    }
  }
  const unifyLinks = unifyNames.map(([key, name], i) => {
    let link =
      i > path.length - 2 ? (
        name
      ) : (
    ...
    return (
      <div key={key} className="unify-link">
        {link}
      </div>
    );
  });
