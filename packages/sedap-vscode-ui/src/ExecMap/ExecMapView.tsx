...
import { BranchCase, DebuggerState, ExecMap } from "@sedap/types";
import TreeMapView, {
  TransformResult,
  TransformFunc,
...
const ExecMapView = ({ state, expandedNodes, toggleNodeExpanded }: Props) => {
  ...
  const cleanNexts = (
    ...
    nexts.map((next) => [{ ...aux, branchCase: next[0] }, next[1][1]]);
  ...
  const transform: TransformFunc<M, D, A> = (map, parent, aux) => {
    ...
    if (map[0] === "Nothing") {
      if (parent === undefined) {
        throw new Error("`Nothing` node has no parent!");
      }
